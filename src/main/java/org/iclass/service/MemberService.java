...
//	암호화 : 평문 -> 알고리즘 -> 암호문(가독X)
//	복호화 : 암호문 -> 알고리즘 -> 평문
//	해싱 : 평문 -> 해싱함수 -> 같은 평문에 대해 가독불가능한 문자열을 생성한다. (암호문)
//		: 암호화된 문자열이 다시 평문으로 복구는 불가능하다.
//		: sha256 해싱함수는 암호문 문자열 256비트 16진수 64개 문자로 만든다.
...
//      3) md.digest() 메소드가 해싱함수 실행. -> 해싱결과가 BYTE[]로 변환
